...
int socket_ops(struct bpf_sock_ops *skops)
{
    ...
    if (e->info.family == AF_INET) {
        ...
        bpf_probe_read_kernel(&e->info.saddr6, sizeof(skops->local_ip6), &skops->local_ip6);
        bpf_probe_read_kernel(&e->info.daddr6, sizeof(skops->remote_ip6), &skops->remote_ip6);
    }
